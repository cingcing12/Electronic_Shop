...
<div class="container my-5">
  ...
  <style>
    ...
    .empty-cart img { width: 220px; opacity: 0.9; margin-bottom: 25px; filter: drop-shadow(0 4px 10px rgba(0,0,0,0.1)); }
    .empty-cart h3 { font-weight: 700; color: #1e1e2f; }
    .empty-cart p { color: #555; font-size: 1.05rem; margin-bottom: 30px; }
    .empty-cart .btn {
      border-radius: 50px;
      font-weight: 600;
      padding: 12px 28px;
        ...
        >
          ...
          <td>
            <div class="input-group mx-auto" style="max-width: 120px;">
              ...
              <input type="text" class="form-control text-center qty-input" value="<?= (int)$it['quantity']; ?>">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateTotal() {
    let total = 0;
    document.querySelectorAll('.subtotal').forEach(el => total += parseFloat(el.textContent));
  ...
  function updateQuantity(productId, newQty, qtyInput, subtotalEl, price) {
  ...
  rows.forEach(row => {
    ...
    btnIncrease.addEventListener('click', () => {
      ...
      updateQuantity(productId, qty + 1, qtyInput, subtotalEl, price);
    });
    ...
    btnRemove.addEventListener('click', () => removeItem(productId, row));

    // Optional: allow manual typing
    qtyInput.addEventListener('change', () => {
      let val = parseInt(qtyInput.value);
      if(isNaN(val) || val < 1) val = 1;
      updateQuantity(productId, val, qtyInput, subtotalEl, price);
    });
  });
